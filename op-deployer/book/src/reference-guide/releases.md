...
contracts. For convenience, the OP Deployer minor version will match the major version of the contracts release.

For example (note that these are just examples, check out the [releases][releases] page for the exact versions to use):

- `v0.2.x`: Supports deploying `op-contracts/v2.0.0`.
- `v0.3.x`: Supports deploying `op-contracts/v3.0.0`.
- `develop`: Supports contracts under active development.
...
contract's tag. For example, if you use `v0.2.x` then the deployment will work as if you were deploying
