...
var Subcommands = cli.Commands{
	...
	},
	{
		...
		Action: func(ctx *cli.Context) error {
			...
			l2Genesis, err := genesis.BuildL2Genesis(config, l2Allocs, genesis.BlockRefFromHeader(l1StartBlock.Header(), nil))
			if err != nil {
				return fmt.Errorf("error creating l2 genesis: %w", err)
			}

			l2GenesisBlock := l2Genesis.ToBlock()
			rollupConfig, err := config.RollupConfig(genesis.BlockRefFromHeader(l1StartBlock.Header(), nil), l2GenesisBlock.Hash(), l2GenesisBlock.Number().Uint64())
			if err != nil {
				return err
			}
