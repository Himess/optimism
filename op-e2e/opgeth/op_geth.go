...
func NewOpGeth(t testing.TB, ctx context.Context, cfg *e2esys.SystemConfig) (*OpGeth, error) {
	...
	l2Genesis, err := genesis.BuildL2Genesis(cfg.DeployConfig, l2Allocs, genesis.BlockRefFromHeader(l1Block.Header(), nil))
	require.NoError(t, err)
	...
	rollupCfg, err := cfg.DeployConfig.RollupConfig(genesis.BlockRefFromHeader(l1Block.Header(), nil), l2GenesisBlock.Hash(), l2GenesisBlock.NumberU64())
	require.NoError(t, err)
